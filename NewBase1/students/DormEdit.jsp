...
<%if(ID==null || OPType.equals("Update")){%>
	...
	<tr>
		...
		<td><input type="text" name="BASE_ORDER" class="textbox" value="<%=sys.getBASE_ORDER()%>"></input></td>
	</tr>
	<tr>
		<td align="right">名称：</td>
		<td><input type="text" name="BASE_NAME" class="textbox" data-options="{required:true,msg:'请输入名称！'}" value="<%=sys.getBASE_NAME()%>"></input></td>
	</tr>
	<tr>
		<td align="right">类型：</td>
		<td><select name="BASE_TYPE1" value="<%=sys.getBASE_TYPE1()%>"><option value="ld"<%=((sys.getBASE_TYPE1().equals("ld"))?" selected":"")%>>楼栋</option><option value="qs"<%=((sys.getBASE_TYPE1().equals("qs"))?" selected":"")%>>寝室</option></select></td>
	</tr>
	...
	<tr>
		<td align="right">类型：</td>
		<td><%=((sys.getBASE_TYPE1().equals("ld"))?"楼栋":"寝室")%></td>
	</tr>
<%}%>
</table>
<%@ include file="../system/buttom.jsp"%>
<%}%>
</body>
</html>
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		try{
			...
			if(rows!=0){//有分页
				...
				o.append("[");
				sys.pstmt=sys.DoParSQL(Tf.select+filedvalue);
				i=0;
				for(String temp:Val){
					sys.pstmt.setString(++i, temp);
			    }
			...
			if(sys.rs!=null){
				...
				while (sys.rs.next()){
					...
					//for(i=1;i<=cols;i++){
					//	if(i>1)o.append(",");
					//	o.append("\"c").append(i).append("\":\"").append(sys.rs.getString(i)).append("\"");
					//}
					o.append("\"id\":\"").append(sys.rs.getString(1)).append("\",\"text\":\"").append(sys.rs.getString(2)).append("\"");
					o.append("}");
				}
				if(rows!=0){o.append("]}");}else{o.append("]");}
			}

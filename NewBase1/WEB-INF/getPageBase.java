...
public class getPageBase extends HttpServlet{
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		try{
			...
			sys.pstmt=sys.DoParSQL(Tf.total +" where base_type=? "+filedvalue);
			i=0;
			...
			sys.pstmt=sys.DoParSQL(Tf.PageRows+" where base_type=? "+filedvalue+" order by sys_id asc,parent_id asc limit ?,?");
			i=0;
			sys.pstmt.setString(++i, ffmodel);
			for(String temp:Val){
				sys.pstmt.setString(++i, temp);
